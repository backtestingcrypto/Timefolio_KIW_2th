...
query = f"""
    SELECT date, ticker_numeric, pct_change_close, pct_change_next_close, pct_change_next2_close,
           bb_upper_break, vol_ma_break, vol_gt_ma5, vol_gt_ma20, ma_order, avg_trade_amount_5d,
    ...
    for group_name, ma_orders in ma_order_groups.items():
        ...
        if results:
            ...
            for _, row in top5.iterrows():
                ...
                subset["weighted_return"] = subset["pct_change_next_close"].clip(-0.3, 0.3).fillna(0)
                ...
                label += f" | {pct_group} | RSI: {rsi_group} | 매매횟수: {len(subset)}"
