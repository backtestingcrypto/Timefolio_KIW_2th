...
def get_en_tickers():
    """sector_code가 'En'인 종목들의 ticker_numeric과 sector_code를 가져옴"""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT ticker_numeric, sector_code FROM kr_stock_data WHERE sector_code = 'En';")
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return rows

def save_en_ohlcv_by_date(start_date: str, end_date: str):
    tickers_info = get_en_tickers()
...
if __name__ == "__main__":
    ...
    save_en_ohlcv_by_date("2018-01-01", today)
